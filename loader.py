...
class VOC2012_Aug_Image_caffe(Dataset):
    def __init__(
        self, 
        root,
        mode='train_aug',
        ...
        if dataaug:
            self.size = size
            self.scales = scales
            self.ColorJitter = T.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5)

        self.filename = [x[:-1] for x in open(root+mode+'.txt')]
    ...
    def __getitem__(self, index):
        ...
        img_path = './VOCdevkit/VOC2012/JPEGImages/{}.jpg'.format(fn)
        #img_path = os.path.join(self.root, img_fn)
        ...
        # Load Annotation
        an_path = 'VOCdevkit/VOC2012/Annotations/{}.xml'.format(fn)
        #an_path = os.path.join(self.root, an_fn)
